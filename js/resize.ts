export function handleResize(embedded = false, fullscreenClass = 'full-page') {
    ...
    const scrollBar = window.innerWidth / window.innerHeight > 560 / 640
        && !fullscreen
        && window.innerHeight > outer.offsetHeight;
    ...
    const landscape = window.innerWidth > 560 * (560 / width);
    const offset = landscape || !scrollBar ? 65 : 55;
    const display = document.querySelector('body .outer #display') as HTMLElement;
    display.style.borderRadius = !landscape || !scrollBar ? "0" : "8px";
    display.style.padding = !landscape || !scrollBar ? "0" : "4px";
    display.style.marginLeft = !landscape || !scrollBar ? "0" : "-7px";
    display.style.borderWidth = !landscape || !scrollBar ? "0" : "3px";
